...
_SYSTEM = (
    ...
    'JSON: {"overall_label": str, "overall_score": float, "aspect_sentiment": '
    'dict, "confidence": float, "intensity": float}.'
)
    ...
    def run(self, input: Discussion, context: RunContext) -> SentimentAssessment:
        result = context.model_gateway.complete(
            ...
            schema_hint=(
                ...
                '"aspect_sentiment": dict, "confidence": float, "intensity": float}'
            ),
            context=context,
        ...
        return SentimentAssessment(
            ...
            aspect_sentiment=dict(result.output.get("aspect_sentiment", {})),
